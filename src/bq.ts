...
export function removeDays(days: number | null, path: string): string {
  return `DELETE
  FROM \`${path}\`
  where CAST(date as DATE) > DATE_SUB( DATE_SUB(CURRENT_DATE("America/Bogota"), INTERVAL 1 DAY), INTERVAL  ${days}  DAY)`;
}


export function updateSchema(schema:string[],path:string):string{
  const fields = schema.join(', ');
  console.log("Actualizare los siguientes campos",fields)
  return `CREATE  OR REPLACE  TABLE   \`${path}\` 
  as select  ${fields} from  \`${path}\``
}
