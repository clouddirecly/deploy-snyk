...
export async function parseBody(body: ParsedJSON): Promise<ArgonOpts> {
  ...
  let fileId: ArgonOpts['fileId'];
  if (
    'fileId' in body &&
    Array.isArray(body.fileId) &&
    body.fileId.length > 0
  ) {
    fileId = body.fileId.map((id: unknown) => Number(id));
  } else {
    fileId = [];
  }
  info(`FileId: ${fileId}`);

  let days: ArgonOpts['days'];
  // eslint-disable-next-line prefer-const
  days = 'days' in body ? Number(body.days) : null;

  info(`days: ${days}`);

  const emailAlertError = body.emailAlertError as EmailAlertError;
...
export async function sendeEmail(data: EmailAlertError) {
  const options: AxiosRequestConfig = {
    url: 'https://us-central1-demos-279017.cloudfunctions.net/sendEmail',
    method: 'POST',
    headers: {
